...
def rpc(chain,method,params,retries=2):
    # A curl/network failure or a JSON-RPC error must not be mistaken for a real
    # "0x" result (that's what `eth_getCode` returns for a genuinely empty
    # address) -- retry the transient case, then surface it as None so the
    # caller can tell "confirmed not a contract" apart from "couldn't ask".
...
def code(chain,a):
    r=rpc(chain,'eth_getCode',[a,'latest'])
    return 'ERR' if r is None else r
def call(chain,a,sel):
    return rpc(chain,'eth_call',[{'to':a,'data':sel},'latest'])
...
def probe(r):
    ...
    for ch in chains:
        c=code(ch,a)
        if c=='ERR': saw_err=True; continue
        if c not in (None,'0x',''):
            return (p,i,a,lab,ch), c, dec_str(call(ch,a,SEL['symbol()']))
    return (p,i,a,lab,chains[0]), ('ERR' if saw_err else '0x'), None

with ThreadPoolExecutor(max_workers=12) as ex:
    results=list(ex.map(probe,uniq))

rpcfail=[]; notcontract=[]; mismatch=[]
for (p,i,a,lab,chain),c,sym in results:
    if c=='ERR':
        rpcfail.append((p,i,a,lab,chain)); continue
    if c in (None,'0x',''):
...
print('\nRPC FAILED to answer, even after retry (inconclusive -- re-run, not a confirmed defect): %d'%len(rpcfail))
for p,i,a,lab,ch in rpcfail: print('   %s:%d  %s  [%s]  label=%r'%(p,i,a,ch,lab))

print('\nNOT A CONTRACT on the stated chain (confirmed -- RPC answered "0x"): %d'%len(notcontract))
...
# `notcontract` is a confirmed defect (exit 1). `rpcfail` means some addresses
# were never actually checked -- exit 0 there would let a "N addresses
# verified" claim stand on addresses that were never verified, so it gets its
# own non-zero code (2) rather than being folded into "pass". mismatch/dupn/dupa
# are advisory -- expected cross-chain/cross-page ticker reuse dominates them
# (see header comment), so they don't gate the exit code.
sys.exit(1 if notcontract else 2 if rpcfail else 0)
