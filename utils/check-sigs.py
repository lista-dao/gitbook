...
for f in md:
    ...
    for blk in re.finditer(r'```solidity\n(.*?)```',txt,re.S):
        ...
        # A `function` inside a `contract X { ... }` body is example code the reader
        # writes, not a protocol call being quoted. Only `interface` members and
        # bare signatures are checked against source.
        skip=[]
        for cm in re.finditer(r'\bcontract\s+\w+[^{]*\{',body):
            d=0
            for j in range(cm.end()-1,len(body)):
                if body[j]=='{': d+=1
                elif body[j]=='}':
                    d-=1
                    if d==0: skip.append((cm.start(),j)); break
