...
# none now. Pass a base ref as argv[2] to enable; the repo carries a long tail of
...
if len(sys.argv) > 2:
    ...
    def _refs(tree):
        seen = {}
        ls = _sp.run(['git','ls-tree','-r','--name-only',tree], capture_output=True, text=True).stdout
        for f in ls.split('\n'):
            if not f.endswith('.md'): continue
            t = _sp.run(['git','show',f'{tree}:{f}'], capture_output=True, text=True).stdout
