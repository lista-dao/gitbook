module.exports = {
  security: {
    label: "安全與審計",
    keywords: [
      "security",
      "audit",
      "audits",
      "audited",
      "auditing",
      "bug bounty",
      "immunefi",
      "vulnerability",
      "vulnerabilities",
      "safe",
      "safety",
      "protect",
      "protection",
      "secure",
      "multi-signature",
      "multisig",
    ],
    filenames: [
      "security/audit-reports.md",
      "security/bug-bounty-immunefi.md",
      "security/lista-lending-risk-fund.md",
      "securing-the-future-an-in-depth-look-at-lista-daos.md",
    ],
  },

  rwa: {
    label: "RWA",
    keywords: [
      "rwa",
      "real-world asset",
      "real world asset",
      "rwa market",
      "treasury",
      "treasury fund",
      "janus henderson",
      "anemoy",
      "usdt.treasury",
      "usdt.aaa",
      "實體資產",
      "真實世界資產",
      "rwa是什麼",
      "什麼是rwa",
    ],
    filenames: [
      "for-developer/rwa/README.md",
      "for-developer/rwa/user-operations.md",
      "for-developer/rwa/bot-operations.md",
      "for-developer/rwa/smart-contract.md",
      "introduction/rwa-markets.md",
      "manual/2025-12-26_Lista-DAO-2025-Annual-Report-3e4ff3f5a085.md",
      "2025-12-26_Lista-DAO-2025-Annual-Report-3e4ff3f5a085.md",
    ],
  },

  lending: {
    label: "Lista Lending",
    keywords: [
      "lista lending",
      "lending",
      "moolah",
      "minloan",
      "morpho",
      "lltv",
      "liquidation loan-to-value",
      "supplier",
      "borrower",
      "vault manager",
      "provider",
      "providers",
      "broker",
      "brokers",
      "re7",
      "re7 vault",
      "re7 labs",
      "mev",
      "mev vault",
      "mev vaults",
      "protocol extension",
      "protocol extensions",
      "interest rate model",
      "irm",
      "adaptivecurveirm",
      "adaptive curve irm",
      "utilization rate",
      "utilisation rate",
      "borrowing rate",
      "usd1 borrowing rate",
      "borrow apy",
      "supply apy",
      "flash loan",
      "market",
      "oracle price feed",
      "lista lending risk fund",
      "lista risk fund",
    ],
    filenames: [
      "for-developer/lista-lending/README.md",
      "for-developer/lista-lending/protocol-extensions.md",
      "for-developer/lista-lending/integration-patterns.md",
      "for-developer/lista-lending/smart-contract.md",
      "for-developer/lista-lending/smart-contract-bsc-core.md",
      "for-developer/lista-lending/smart-contract-bsc-credit.md",
      "for-developer/lista-lending/smart-contract-bsc-brokers.md",
      "for-developer/lista-lending/smart-contract-bsc-oracles.md",
      "for-developer/lista-lending/smart-contract-bsc-smart-lending.md",
      "for-developer/lista-lending/smart-contract-ethereum.md",
      "for-developer/services/README.md",
      "for-developer/services/lending-api/README.md",
      "for-developer/services/lending-api/overall.md",
      "for-developer/services/lending-api/market.md",
      "for-developer/services/lending-api/vault.md",
      "for-developer/services/lending-api/position-liquidation-emission.md",
      "for-developer/services/liquidation-logic.md",
      "for-developer/services/position-data-maintenance.md",
      "for-developer/services/subscription-module.md",
      "introduction/lista-lending/README.md",
      "introduction/lista-lending/borrowers-and-suppliers.md",
      "introduction/lista-lending/liquidation/README.md",
      "introduction/lista-lending/liquidation/liquidation-zone.md",
      "introduction/lista-lending/markets.md",
      "introduction/lista-lending/fixed-rate-and-term-loans.md",
      "introduction/lista-lending/third-party-vault-risk-management.md",
      "introduction/lista-lending/user-flow.md",
      "introduction/lista-lending/oracle.md",
      "introduction/lista-lending/interest-rate-model-irm.md",
      "introduction/lista-lending/fees.md",
      "introduction/lista-lending/flash-loan.md",
      "introduction/lista-lending/vaults/README.md",
      "user-guide/lista-lending/how-to-create-a-vault-lista-lending-vault-gui.md",
      "security/lista-lending-risk-fund.md",
      "introducing-lista-lending-lista-daos-next-gen-lend.md",
      "product-guide-lista-lending.md",
      "product-update-lista-lending-vault-manager-gui.md",
      "product-update-lista-lending-alpha-zone-powering-t.md",
      "product-update-introducing-lista-daos-liquidation-.md",
      "product-update-mint-clisbnb-in-lista-lending.md",
      "Guide Running Lista's Liquidation bot.md",
      "Product Guide Borrowing USD1 on Lista Lending.md",
      "Product Guide Lista Lending.md",
      "Product Update Lista Lending Vault Manager GUI.md",
      "The Re7 Vault Forced Liquidation A Recap Moving Forward.md",
      "What makes Lista and lisUSD different.md",
    ],
  },

  cdp: {
    label: "CDP / lisUSD",
    keywords: [
      "cdp",
      "collateral debt position",
      "lisusd",
      "slisbnb collateral",
      "bnb validator",
      "liquid staking",
      "staking",
      "mcr",
      "minimum collateral ratio",
      "dutch auction",
      "borrowing fee",
      "stability fee",
    ],
    filenames: [
      "introduction/collateral-debt-position-lisusd/README.md",
      "introduction/collateral-debt-position-lisusd/collateral/README.md",
      "introduction/collateral-debt-position-lisusd/collateral/loan-liquidation.md",
      "introduction/collateral-debt-position-lisusd/lisusd/README.md",
      "introduction/collateral-debt-position-lisusd/technical-guide.md",
      "for-developer/collateral-debt-position/README.md",
      "for-developer/collateral-debt-position/mechanics.md",
      "for-developer/collateral-debt-position/flash-loan.md",
      "for-developer/collateral-debt-position/multi-oracle.md",
      "user-guide/collateral-debt-position/README.md",
      "user-guide/collateral-debt-position/borrow-lisusd.md",
      "user-guide/collateral-debt-position/provide-collateral.md",
      "user-guide/collateral-debt-position/repay-lisusd.md",
      "user-guide/collateral-debt-position/withdraw-collateral.md",
      "introduction/liquid-staking-slisbnb/README.md",
      "introduction/liquid-staking-slisbnb/about-slisbnb.md",
      "introduction/liquid-staking-slisbnb/technical-guide.md",
      "introduction/bnb-validator-lista-dao.md",
      "Product Guide Looping strategies with lisUSD.md",
      "What makes Lista and lisUSD different.md",
    ],
  },

  smartLending: {
    label: "Smart Lending",
    keywords: [
      "smart lending",
      "smart-lending",
      "smartlending",
      "dex lp",
      "liquidity pool",
      "lp token",
      "impermanent loss",
      "fixed ratio",
      "custom ratio",
      "smart swap",
      "smart-swap",
      "liquidation",
    ],
    filenames: [
      "introduction/smart-lending.md",
      "user-guide/smart-lending.md",
      "ListaDAO Smart Lending A Hands-on Tutorial.md",
      "ListaDAO’s Smart Lending A Hands-on Tutorial.md",
      "The Ultimate Guide to Lista Smart Swap.md",
      "Everything You Need to Know About Liquidation on Lista Smart Lending.md",
      "Product Guide LP Farming with lisUSD:USD1 on Lista DAO.md",
    ],
  },
};
