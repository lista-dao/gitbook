...
Oracle是一种智能合约，它为区块链应用程序提供外部数据，尤其是价格信息。在像Lista Lending这样的借贷协议中，oracle应提供具有8位小数精度的价格数据。例如，如果1 BTC的价格是80,000，它将返回80,000 * 100,000,000 = 8,000,000,000,000。
...
所有在Lista Lending市场中使用的oracle都实现了IOracle接口，该接口有一个标准化的函数：
...
可以使用各种oracle实现与Lista Lending市场：
...
* Morpho协会或仓库的作者不承担因使用此信息而可能导致的任何损失或损害的责任。
