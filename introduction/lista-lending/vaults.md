...
Lista Vault 支持单一贷款资产，并将存入资金分配到多个 Lista 市场。通过向金库存款，用户可以从借款人支付的利息中赚取被动收益。
...
Lista Lending 将利用一篮子不同的预言机，备有备份，以确保通过预言机定价大大减少风险价格操纵
...
金库所有者可以选择设置时间锁，以管理关键参数更改和风险管理过程。
...
Lista Lending 允许在 BNB Chain 上无需许可地创建借贷金库，使策展人能够部署自定义流动性池，以优化收益并管理风险。Lista Vault 接受单一贷款资产（例如 lisUSD）并将存款分配到多个 Lista 市场。本指南将引导您完成部署和配置金库的过程，使您能够在 Lista DAO 的生态系统内策划借贷机会。
...
* BNB Chain 钱包：配置了 BNB Chain 的钱包（例如 MetaMask），并有足够的 BNB 用于支付燃气费。
* 开发环境：具备 Solidity 开发设置（例如 Hardhat, Foundry）并安装了 Node.js。
* 贷款资产：用作金库贷款资产的 ERC-20 代币（例如 lisUSD, BNB）。
* 市场知识：熟悉 Lista Lending 市场（贷款/抵押品对），以有效分配流动性。
* 预言机选择：Chainlink（主要）、Binance Oracle（比较）、Redstone（备份）用于价格提供。
...
* indexes (uint256\[]): 重新排序现有提款队列的索引数组。\
  \
...
在开始部署之前，请确保您已决定这三个地址。您应该在表格中填写实际要使用的地址。

#### 步骤  4.2：部署时间锁合约

导航到[ https://github.com/lista-dao/moolah](https://github.com/lista-dao/moolah) 存储库并切换到 feature/deployScripts 分支。
...
确保您的部署钱包中有足够的资金来支付燃气费，并且您已正确配置环境变量以进行网络和验证服务的认证。
...
2. 测试存款
   * 存入少量贷款资产（例如 10 lisUSD）以确保金库按预期功能。
   * 通过 Lista Lending 界面检查分配和 APY。
3. 上线
   * 向社区宣布您的金库（例如 Lista DAO Discord）以吸引存款人。

#### 部署后管理

* 监控表现：通过 Lista Lending 仪表板跟踪 APY 和市场条件。
* 重新平衡流动性：根据需要使用分配者角色调整分配。
* 必要时升级：利用可升级设计修改参数或添加功能（需要所有者权限）。
