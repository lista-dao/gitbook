...
Items in both arrays carry `tokenAddress` (lowercase), `tokenSymbol` and `tokenIcon`, plus one USD figure typed **`number`, not a string**: `amountInUSD` (deposited) on `loanTokens`, `liquidityInUSD` (available loan-side liquidity in markets using that collateral) on `collateralTokens`.
...
| Field | Type | Description |
|-------|------|-------------|
| `totalCollateral`, `totalBorrowed`, `totalAvailable` | string | bStock collateral posted, borrowed against it, and available liquidity — all USD, fixed-point 18 decimals. |
| `marketCount`, `collateralTokenCount` | number | bStock markets, and distinct bStock collateral tokens. |
| `loanTokens`, `topCollateralTokens` | array | Loan tokens available against bStock collateral, and the top bStock collaterals. |
