...
| 6. Repayment | `repay(amount, posId, onBehalf)` / `repayAndWithdraw(...)` / `repayInterestWithLista(...)` (currently disabled — see below) | Interest is repaid first, then principal. Interest (and any penalty) is supplied to the Credit Vault via the relayer. Emits `RepaidFixedLoanPosition`. |
...
| 8. Penalty window | After `end + graceConfig.period` (`dueTime`) | The position is "penalized." A penalty of `penaltyRate × (remaining principal + accrued interest)` is required, and the position **must be repaid in full** in a single repayment. Current penalty rate: **3%** (`graceConfig.penaltyRate = 3 * 1e25`, read live from `graceConfig()`). Note the `initialize()` default in the source is 15% — it was lowered by the MANAGER, so do not read the constructor value. |
