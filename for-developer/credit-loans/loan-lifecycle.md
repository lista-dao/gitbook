...
| 8. Penalty window | After `end + graceConfig.period` (`dueTime`) | The position is "penalized." A penalty of `penaltyRate × (remaining principal + accrued interest)` is required, and the position **must be repaid in full** in a single repayment. `graceConfig.penaltyRate` is `RATE_SCALE`-scaled (`1e27`), so `3 * 1e25` reads as 3%; `setGraceConfig` caps it at `RATE_SCALE`. Read it from `graceConfig()`; the `initialize()` default in the source is not what is deployed. |
