...
| **Lista Rights** | Reward distribution for the **holder** emission programme. `LendingRewardsDistributorV2` and `RewardsRouter` are each deployed more than once, one per emission programme, and the address tables do not always say which. Identify the contract by the page it sits on rather than by its name: this page for holder emission, [BSC Core](lista-lending/smart-contract-bsc-core.md) for lending and bStock, [BSC Credit](lista-lending/smart-contract-bsc-credit.md) for credit. | [README](lista-rights/README.md) | [Smart Contract](lista-rights/smart-contract.md) |
