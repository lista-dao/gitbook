...
The Moolah Lending SDK is the canonical TypeScript integration path for Lista Lending (Moolah) on BNB Chain and Ethereum. It is published on public npm under the `@lista-dao` scope.
...
| Package | Latest version (as of 2026-09) | Description |
| --- | --- | --- |
...
The SDK targets ES modules (`"type": "module"`) and ships its own type declarations. `viem` is a **direct dependency** of both packages (`viem@^2.22.10`), so you do not need to install it to use the SDK. You will use viem directly to execute the returned steps, though — install a version that satisfies the same range so your package manager dedupes to a single copy. Two viem instances cause client and type mismatches across the `publicClients` boundary.
...
| `publicClients` | `Record<string, PublicClient>` | Supply your own viem `PublicClient` per chain ID; where one is present the SDK uses it and never builds a client from `rpcUrls` for that chain. Note `rpcUrls` is still required by the config type — pass `rpcUrls: {}` if you supply clients for every chain you use. |
| `transport` / `transportByChainId` | `SdkTransportConfig` | Optional `timeout`, `retryCount`, `retryDelay` tuning, globally or per chain ID. |

> **Networks.** Lista Lending runs on BNB Chain (chain ID `56`) and Ethereum (chain ID `1`), and the SDK accepts a chain ID anywhere a `chainId` argument is shown below.
>
...
| `getBrokerFixedTerms(chainId, broker)` | Chain | `FixedTermAndRate[]` — available fixed terms/rates for a broker. Returns `{termId, duration, apr}` only — it decodes with the `LendingBroker` ABI, so it does not support a `CreditBroker`, whose struct carries a fourth field (`termType`). |
...
for (const step of borrowSteps) {
  // `params.to` must be mapped onto viem's `address`; `params.chainId` and
  // `params.data` are extra fields that `writeContract` does not accept.
...
// preview.simulation -> projected collateral, borrowed, LTV, loanable,
//                       baseLoanable, computeLoanableForLTV, liqPriceRate
```
...
// result -> { collateral, borrowed, LTV, loanable, baseLoanable,
//             computeLoanableForLTV, liqPriceRate, borrowRate? }
```
...
const raw = new Decimal(123456000000000000000n, 18); // same value from raw bigint + decimals
