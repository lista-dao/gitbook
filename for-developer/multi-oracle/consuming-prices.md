...
Bounds are configured per asset — `BoundValidator` stores no default, so consult the per-asset limits in the auto-synced [Standard Collaterals](../multi-oracle-standard.md) and [bStock Collaterals](../multi-oracle-bstock.md) tables. A reported price of `0` fails validation gracefully (returns `false`); an anchor price of `0` instead reverts (`anchor price is not valid`). An asset that has a **pivot oracle enabled but no bound-validator config** reverts with `validation config not exist`, and that revert propagates out of `peek` — it is raised inside the success branch of the main-oracle read, so the fallback `catch` does not absorb it. Do not confuse this with an asset that has no resilient-oracle config at all, which reverts `invalid resilient oracle price` instead.
