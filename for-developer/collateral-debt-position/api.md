...
Read-side endpoints for the **lisUSD CDP**, where each position is backed by a single collateral type (`ilk`). They are keyed by `ilk` (collateral type) and collateral token address, not by a Moolah `marketId`.
