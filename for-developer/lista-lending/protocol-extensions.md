...
Moolah oracles expose `peek(address)`. Every oracle Lista deploys on the `IOracle` path except `IdleOracle` (which returns a literal `0` for its idle collateral) returns an 8-decimal price — the legacy CDP `peek()` surface is 18-decimal and Moolah never calls it, but that is a property of those deployments, not something the interface declares or Moolah validates — see [Consuming Oracle Prices](../multi-oracle/consuming-prices.md) before trusting a scale on a market whose oracle you did not deploy.
