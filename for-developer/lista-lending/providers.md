...
To find out which provider backs which collateral, see [Integration Patterns](integration-patterns.md); for deployed addresses, [Smart Contract](smart-contract.md).
...
Each instance is **bound to one vault**, so pick the one deployed for the vault you are targeting.
...
slisBNB collateral is provider-gated, so these are the only ways *you* can voluntarily move it — liquidation is a separate path that seizes it directly, bypassing the provider entirely:
...
For Smart Lending's provider, go to [Smart Lending & StableSwap](stableswap-integration.md) — it covers the supply/withdraw variants, the price-difference guard and the collateral token's transfer restrictions.
