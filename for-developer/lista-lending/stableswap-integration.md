...
> The collateral token is **not a freely transferable ERC-20**. `mint` and `burn` are restricted to the minter, and `transfer` / `transferFrom` are restricted to Moolah and addresses holding the `TRANSFERER` role. Do not build a flow that moves it between arbitrary accounts — route position changes through the provider.

Route swaps through Lista StableSwap, or use an LP position as Moolah collateral:
...
For deployed addresses, see [BSC Smart Lending](smart-contract-bsc-smart-lending.md) (do not hard-code addresses from this page — the address tables are the source of truth). For how providers fit into Moolah, see [Integration Patterns](integration-patterns.md).
...
To keep an up-to-date pool list, index the factory's `NewStableSwapPair` event, or walk `swapPairContract(i)` over `[0, pairLength)`.
...
Each pool holds a resilient-oracle reference and **can** enforce a price-difference guard on state-changing operations — but whether it does is per-pool and manager-controlled. Read `skipPriceDiff()` on the specific pool rather than assuming the guard protects you — `true` means the guard is **off**.
...
| Skip flag | `skipPriceDiff() → bool` | When `true`, the guard is not enforced. One older registered pool predates this function and **reverts** when you call it — treat a revert as "guard behaviour unknown", not as `false`. |
...
Both paths emit the provider's `SupplyCollateral` event.

Once collateral is in the market, borrowing and repayment happen against the Moolah market as usual. Withdrawal has four paths, not a one-to-one mirror of the two supply entry points: the provider exposes `withdrawDexLp` (return LP), `withdrawCollateral` (proportional token0/token1), `withdrawCollateralImbalance` (exact token amounts), and `withdrawCollateralOneCoin` (single coin) — one for each way collateral can be split on exit. All four burn the collateral token, but only the latter three call `remove_liquidity*` on the pool and are therefore subject to the price-difference guard. **`withdrawDexLp` returns the raw LP token without touching the pool**, so it never hits the guard — it is the exit that stays available when the guard would otherwise block a withdrawal.
