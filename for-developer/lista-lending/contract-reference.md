...
**Moolah** is the singleton that holds every Lista Lending market. Call it directly from Solidity, or from any stack that cannot use the [SDK](../sdk.md); the signatures, structs and Lista-specific controls below are what you need to do that.

Moolah is powered by Morpho and built on the Morpho Blue smart contracts, then extended with Lista-specific controls. For the layers around it: the ERC-4626 vault is in [Vault Reference](vault-reference.md), provider-gated collaterals in [Providers](providers.md), and the fixed-term broker surface in [Broker Reference](broker-reference.md). Every market — regardless of collateral, oracle, or IRM — lives inside this one contract and is addressed by a market `Id`. For deployed addresses, see the [Smart Contract](smart-contract.md) reference; For higher-level flows see [Integration Patterns](integration-patterns.md); for the callback interfaces and emitted events see [Events & Callbacks](events-and-callbacks.md).
...
| `oracle` | `address` | Price source; must expose `peek(address)`. Every oracle Lista deploys on the `IOracle` path except `IdleOracle` (which returns a literal `0` for its idle collateral) returns an 8-decimal price — the legacy CDP's own `peek()` surface is 18-decimal, but that's a separate, unrelated contract. Moolah itself never validates the decimals convention either way: market creation merely probes that `peek` does not revert — see [Consuming Oracle Prices](../multi-oracle/consuming-prices.md). |
...
Where a function takes both `assets` and `shares`, **exactly one must be zero** (enforced by `exactlyOneZero`); the other side is derived by the shares math.
...
> **Approve the contract that actually pulls the tokens — that isn't always Moolah.** For direct core calls with no provider or broker in the way, `supply` and `supplyCollateral` pull with `transferFrom` where the spender is the Moolah singleton, so approve Moolah for those. Gating is per leg, not per market: a **collateral provider** takes over `supplyCollateral` / `withdrawCollateral`; a **broker** takes over `borrow` / `repay`. A broker does not gate collateral — on a broker market with no collateral provider you still call `supplyCollateral` on Moolah and approve Moolah for it. Two cases route the pull elsewhere: when a **collateral provider** is set, Moolah requires `msg.sender == provider`, so the provider itself calls `supplyCollateral` and must have already pulled the tokens from you (e.g. `SmartProvider`/`SlisBNBProvider` both do `transferFrom(msg.sender, address(this), ...)` before calling Moolah) — **approve the provider, not Moolah**. When a **broker** is set, `Moolah.repay` requires `msg.sender == broker`, so the end user cannot call it directly — the broker itself pulls the repayment from the user (e.g. `LendingBrokerOperatorLib` does `transferFrom(user, address(this), ...)`) before forwarding to Moolah — **approve the broker contract, not Moolah, for repay on a broker market.** See [Integration Patterns](integration-patterns.md).
...
These are compile-time constants of the interface, not adjustable parameters — no governance call changes them.
...
Markets read prices through the `oracle` in `MarketParams`, which exposes `peek(address asset)`. Each market names its own oracle, so which contract answers — a **Resilient Oracle** aggregating a main / pivot / fallback source per asset, or an asset-specific adaptor — is per-market configuration; read `marketParams.oracle` and check it against the [Standard Collaterals](../multi-oracle-standard.md) / [bStock Collaterals](../multi-oracle-bstock.md) tables. When a broker is present and a user address is supplied, price resolution routes through the broker so the effective price reflects the user's fixed-term/fixed-rate position; health checks otherwise use the plain oracle price. Oracle selection is a per-market safety responsibility. See [Consuming Oracle Prices](../multi-oracle/consuming-prices.md).
...
Pause is the privileged control that can halt every `whenNotPaused` entry point: a `PAUSER` can stop supply, withdraw, borrow, repay, collateral movement, liquidation and `flashLoan`. Views are unaffected. Handle its revert. It is not the only state that can change under you, though — `providers`, `brokers`, and `minLoanValue` are separate mappings/variables, not part of `MarketParams` or `Market`, so a `market()` or `idToMarketParams()` read does not tell you whether they changed either; re-read them, or watch [Events & Callbacks](events-and-callbacks.md), rather than assuming a cached read still holds. `createMarket` is additionally gated whenever any `OPERATOR` exists.

Everything else — enabling IRMs and LLTVs, setting fees, whitelists, providers, brokers, `minLoanValue`, and contract upgrades — is role-gated, so a third party cannot call any of it. Upgrade authority sits behind an OpenZeppelin `TimelockController` — read `getMinDelay()` on it for the delay in force.
