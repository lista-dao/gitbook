...
| `oracle` | `address` | Price source; must expose `peek(address)`. Every oracle Lista deploys on the `IOracle` path except `IdleOracle` (which returns a literal `0` for its idle collateral) returns an 8-decimal price — the legacy CDP `peek()` surface is 18-decimal and Moolah never calls it, but Moolah never validates it — market creation merely probes that `peek` does not revert — see [Consuming Oracle Prices](../multi-oracle/consuming-prices.md). |
