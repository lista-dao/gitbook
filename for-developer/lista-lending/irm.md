...
# then bounded again by the market's own cap / floor before being stored:
endRateAtTarget  = clamp(endRateAtTarget, rateFloor * 4, cap / 4)
```

The second clamp is applied to the value that is actually persisted, so after a `rateCap` or `rateFloor` change the stored `rateAtTarget` is bounded to `[rateFloor * 4, cap / 4]` — not only to the protocol-wide `MIN_RATE_AT_TARGET` / `MAX_RATE_AT_TARGET`. The factor of 4 is `CURVE_STEEPNESS`, so the bound expresses the cap/floor in terms of the curve endpoints. If `rateFloor * 4` exceeds `cap / 4`, the cap side wins.
...
Changing either bound settles first: `updateRateCap` and `updateRateFloor` (on both IRMs, and `setBorrowRate` on `FixedRateIrm`) call `Moolah.accrueInterest` for the market before writing the new value, so interest accrued under the old bound is booked at the old bound. The call is a no-op for a market that has never been touched (`lastUpdate == 0`).

## Adaptive-curve constants

These are compiled into `ConstantsLib` and inlined, so there is no getter to read them from — changing one requires a contract upgrade. All rate constants are **per second, WAD-scaled**; the percentages in *Meaning* are those same values expressed per year.
...
Because `borrowRate` requires `msg.sender == MOOLAH`, integrators reading a rate directly should call `borrowRateView`. The value it returns is the average per-second rate that *would* be applied for the given `market` snapshot; it does not advance `rateAtTarget`. `borrowRateView` uses the stored `rateAtTarget` and the elapsed time implied by `market.lastUpdate`, so its result already reflects adaptation that has not been committed on-chain.
...
| `setBorrowRate(Id id, int256 newBorrowRate)` | — | Sets the fixed rate. Access-controlled (bot role); must be `>= 0` and `<= MAX_BORROW_RATE`, and within any configured `rateCap` / `rateFloor`. Interest accrued under the previous rate is settled first (see below). |
...
A `0` rate means no interest accrues, so treat an unset market as unconfigured rather than free. This IRM backs the fixed term / fixed rate broker products described in [Integration Patterns](integration-patterns.md).

## Related

* [Contract & Interface Reference](contract-reference.md) — `minLoan`, the global reentrancy guard and the rest of the Lista-specific controls.
