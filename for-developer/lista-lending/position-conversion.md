...
A borrower with a variable-rate position on a Moolah market can move it into a **fixed-term** market without unwinding — no repay, no re-borrow, no flash loan. `PositionManager` does it atomically.
...
> **One direction only.** There is no fixed → variable path. To leave a fixed term early, repay the fixed position through its broker and borrow again on the variable market. See [Broker Reference](broker-reference.md) for the repay surface.
