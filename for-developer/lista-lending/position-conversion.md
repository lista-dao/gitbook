...
Before you offer the conversion, check two things that make the call revert: Moolah must have `borrowAmount` of the loan token free for the flash loan, and it must not be globally paused.
...
Broker markets run a wide LLTV spread, so migrating out of a high-LLTV variable market into a lower-LLTV fixed one reverts. Compare `lltv` on both [market rows](smart-contract-bsc-brokers.md) before offering the conversion.

## Finding the target market and term

You have to supply these two yourself — the call cannot derive either:
...
- [Moolah Lending SDK](../sdk.md) — this flow has no SDK builder; assemble the call directly.
