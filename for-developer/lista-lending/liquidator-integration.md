...
> **This is the part integrators get wrong.** On a market that is gated at the Moolah level, you cannot liquidate an arbitrary under-water position — the position has to have been surfaced through condition 2 or 3 first. The gating is *not* a health check: a position failing `isLiquidatable` reverts with `NotWhitelisted()` even when it is deeply under water. Query the [`/api/liquidation/zone/list`](../services/lending-api/position-liquidation-emission.md) feed, which returns exactly the positions that are currently on the whitelist, rather than deriving your own candidate list from raw market state.
