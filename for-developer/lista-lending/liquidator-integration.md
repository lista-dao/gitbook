...
To run a **third-party liquidator** against Lista Lending (Moolah) you need four things: which contract to call, how eligibility is gated, how to size the liquidation, and how the collateral comes back to you.
...
Moolah deploys three liquidator contracts. `PublicLiquidator` is the third-party integration surface.
...
function isLiquidatable(bytes32 id, address borrower) internal view returns (bool) {
  return
    IMoolah(MOOLAH).isLiquidationWhitelist(id, address(0)) ||  // 1. cheap probe — NOT proof the market is open; see below
...
Answer these two separately — one does not imply the other:
...
> On a market that is gated at the Moolah level, an under-water position must first be surfaced through condition 2 or 3. The gating is *not* a health check: a position failing `isLiquidatable` reverts with `NotWhitelisted()` even when it is deeply under water. Select the feed that matches the gate. `/api/liquidation/zone/list` returns the borrower **whitelist**, so it finds candidates on *gated* markets and may be empty for open markets. For open markets use [`/api/moolah/redPositions`](../services/lending-api/position-liquidation-emission.md), which takes a single market `id` — enumerate markets from `/api/moolah/allMarkets` and fan out.
...
For standard (non-broker) markets, `PublicLiquidator` exposes a public view that reproduces Moolah's liquidation sizing:
...
**Approve at least this amount of the loan token to `PublicLiquidator` before calling a self-funded path.** Any unused loan token is refunded in the same transaction. The quoted amount therefore covers the expected repayment; a small buffer may be needed where the quote can change before execution.

> **Do not use this quote on a broker market.** `loanTokenAmountNeed` prices the collateral with `Moolah.getPrice(marketParams)`, which is the plain market price (`user = address(0)`). `Moolah.liquidate` instead prices with `_getPrice(marketParams, borrower)`, and on a market with a broker that routes to the broker and can deviate from the market price according to the borrower's position. The broker's collateral price is the market price minus a deduction, so it is never higher — the quote therefore **over**-states the repayment. `PublicLiquidator` pulls the larger amount and refunds the surplus in the same transaction, so the call succeeds, but you must hold the inflated figure to make it. On broker markets, derive the repayment from the broker's own pricing — `peek(token, user)` and `getUserTotalDebt(user)`, see [Broker Reference](broker-reference.md).
...
| Moolah `"position is healthy"` | The position was healthy by the time your transaction landed — for example, because someone repaid or the price moved. Re-check immediately before submitting and expect execution races. |
