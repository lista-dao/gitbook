...
Moolah hands control back to you in two ways:
...
Signatures and indexed parameters are in `moolah/libraries/EventsLib.sol` — read them from there or from the ABI. `Id` is the market identifier (the keccak hash of `MarketParams`). Five things will catch you out when reconstructing state from these logs:
...
- **Repay/Liquidate rounding.** The repaid amount — `assets` on `Repay`, `repaidAssets` on `Liquidate` — may exceed the market's `totalBorrowAssets` by 1 due to rounding. Account for this when reconciling.
...
The ERC-4626 curator vault layer emits the standard `Deposit`/`Withdraw`/`Transfer` events plus its own lifecycle and configuration set, declared in `moolah-vault/libraries/EventsLib.sol`. Two declared event groups require special handling:

**Do not index the vault's `Submit*` / `Revoke*` events.** `SubmitCap`, `SubmitTimelock`, `SetTimelock`, `SubmitGuardian`, `SetGuardian`, `SubmitMarketRemoval` and the `RevokePending*` family are all declared in `EventsLib` but emitted **nowhere**. They are inherited declarations; Lista uses two external OpenZeppelin `TimelockController`s per vault instead of the in-contract pending/timelock mechanism. Their addresses appear in `CreateMoolahVault` as `managerTimeLock` and `curatorTimeLock`; watch those for `CallScheduled` / `CallExecuted` / `Cancelled`. `SetCap` is emitted directly by the vault, with no preceding `SubmitCap`.

`SetCurator` and `SetIsAllocator` are declared in the same library and likewise never emitted. The vault uses OpenZeppelin `AccessControl`, so role changes surface as `RoleGranted` / `RoleRevoked` filtered on the `CURATOR` / `ALLOCATOR` role hashes.
...
The public reallocation helper (`src/vault-allocator/VaultAllocator.sol`) declares its events in `vault-allocator/libraries/EventsLib.sol`.
