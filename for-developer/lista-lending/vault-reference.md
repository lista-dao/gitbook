...
`MoolahVault` is the ERC-4626 layer over Moolah: suppliers deposit one loan asset, a curator allocates it across markets, and shares accrue the blended yield. Call a vault **directly** — from Solidity, or from any stack that cannot use the [SDK](../sdk.md) — with the surface below.
...
Exceeding the reachable amount reverts `NotEnoughLiquidity()`. Both views can **under-report** because of share/asset rounding, so treat them as a safe lower bound. **Do not present a user's full share balance as withdrawable** — quote `maxWithdraw` instead.
