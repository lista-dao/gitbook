...
| Method | Description |
| --- | --- |
| `deposit(uint256 amount, uint256 shares, address receiver)` | Mints shares to `receiver` and transfers assets from `msg.sender` to the adapter. Accepts **either** `amount` or `shares` — pass the one you want to specify and `0` for the other; the pool derives the second from the first. Two gates to know: a **minimum deposit** (`minDeposit`, in asset units) that reverts `deposit below minimum` — a small test deposit will fail; and a receiver whitelist that is **open while the set is empty**; once a manager populates it, unlisted deposits and share transfers revert. Read `minDeposit()` and `getWhiteList()` before assuming. |
| `requestWithdraw(uint256 amount, uint256 shares, address receiver)` | Same dual `amount`/`shares` convention as `deposit` — pass one and `0` for the other. Takes a withdrawal fee in shares (transferred to the fee receiver — read `withdrawFeeRate()`, which is `1e18`-scaled, so `1e15` is 0.1%), burns the remainder from `msg.sender`, and records the receiver and the amount — which is re-derived from the reduced share count, so the queued payout is below a naive `convertToAssets(shares)`. |
| `claimWithdraw` | Transfers assets to receiver after adapter-funded liquidity is available in `RWAEarnPool`. |

## Withdrawal Lifecycle

Withdrawal is asynchronous. The settlement leg runs on the **external vault's redemption cycle**, not on a schedule the contract enforces, so there is no on-chain deadline by which a pending request becomes claimable. Poll for claimable liquidity rather than assuming a fixed delay. Interest accrues into share NAV as it is reported, so a share's value grows over the holding period rather than paying out separately.
