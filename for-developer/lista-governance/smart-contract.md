...
> **veLISTA is being retired under LIP-024.** The on-chain switch is `freePenaltyStartTime` = 2026-04-07 08:20 UTC; from that point `lock` / `increaseAmount*` / `extendWeek` revert and `getPenalty` returns `0`. `lock`, `relockUnclaimed`, `increaseAmount`/`increaseAmountFor` and `extendWeek` all revert, though `enableAutoLock` / `disableAutoLock` stay callable on existing positions. Existing locks can be exited without penalty via `claim()` or `earlyClaim()` — there is no `withdraw` function — while `getPenalty()` returns `0`. Two caveats: the penalty-free period is a bounded window (`freePenaltyEndTime`, currently set to year 9999) that a `MANAGER` can change, and `earlyClaim` has an independent `earlyClaimBlacklist` gate; governance voting has moved to plain LISTA on Snapshot; protocol revenue previously distributed to veLISTA stakers now funds LISTA buybacks. The `veLista*` contracts below are kept for reference — they remain readable on-chain and existing positions can still be exited through them — but they should not be used for new integrations.
