...
> **veLISTA is being retired under LIP-024.** The on-chain switch is `freePenaltyStartTime` = **2026-04-07 08:20 UTC**. From that moment `lock`, `relockUnclaimed`, `increaseAmount` / `increaseAmountFor` and `extendWeek` all revert with `free penalty period start`, and `getPenalty(address)` returns `0` for every account **while `block.timestamp` is within `[freePenaltyStartTime, freePenaltyEndTime]`** — see the bounded-window caveat below before relying on it past that. `enableAutoLock` / `disableAutoLock` remain callable on existing positions. Governance voting has moved to plain LISTA on Snapshot, and protocol revenue previously distributed to veLISTA stakers now funds LISTA buybacks.
>
