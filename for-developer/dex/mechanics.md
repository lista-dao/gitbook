...
> **Lista V3 is a fork of Uniswap V3.** The canonical Uniswap V3 model, math, contract interfaces, and SDKs (e.g. `@uniswap/v3-sdk`, `@uniswap/sdk-core`) apply directly. Contracts are renamed (`ListaV3Factory`, `ListaV3Pool`) but their interfaces, events, and behaviour match Uniswap V3. Four things differ in Lista's deployment and will break a copied V3 integration if you miss them: the enabled fee tiers, the NFT name, an upgradeable position manager, and — for off-chain address derivation — a Lista-specific pool init-code hash (see [Building on Lista V3](#building-on-lista-v3)). For the underlying math, use the Uniswap V3 whitepaper.
...
> Lista's stablecoin and LST pools (noted in the table above) use the two lowest tiers, which are **not** in the constructor set. Do not assume that Uniswap's common `500` / `3000` / `10000` tiers identify Lista pools; query the factory for the target pool's fee tier.
